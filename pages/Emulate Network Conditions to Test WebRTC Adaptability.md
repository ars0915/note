...
- # Test with Network Emulator
	...
	- **High Latency**
	  * Introduce delays (e.g., 100ms, 300ms, 500ms) to emulate high-latency environments like satellite or long-distance networks.
	  * Check how latency affects ICE candidate negotiation and media sync.
	- **Jitter Simulation**
	  * Add variable latency (jitter) to see how WebRTC compensates for irregular delays.
	  * Monitor jitter buffer size and its impact on audio and video playback.
	- **Packet Loss**
	  * Introduce packet loss rates (e.g., 1%, 5%, 10%) to emulate unreliable networks.
	  * Evaluate the impact on video freezes, audio glitches, and DataChannel reliability.
