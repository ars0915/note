...
- # MQ
	...
	- #### C. 使用場景對比
	  ...
	  **RabbitMQ 適合：**
	  ...
	  1. **Task Queue**
		...
		- 你在 CHATISFY 的 message sending system 就是這種
	...
	- #### 你在 CHATISFY 的使用
	  ...
	  **RabbitMQ (Message Sending System):**
	  ```
	  使用場景：
	  - 每個 fanpage 的訊息發送
	  - 需要保證訊息送達
	  ...
	  你的創新：
	  - Dynamic queue per fanpage
	  - 避免互相 blocking
	  - Redis Pub/Sub 做 cleanup
	  
	  為什麼用 RabbitMQ 而非 Kafka:
	  - 需要複雜的 routing（不同 fanpage）
	  - 需要 acknowledgment 保證送達
	  - 不需要 replay messages
	  - Lower latency 對 messaging 很重要
	  ```
	- ### 面試回答模板
	  
	  **被問：「RabbitMQ 和 Kafka 的差異？」**
	  
	  **回答（1-2分鐘）：**
	  "這兩個工具設計理念不同：
	  
	  RabbitMQ 是傳統的 message broker，適合做 task queue 和複雜路由。它用 push model，消費後訊息就刪除。比較適合需要保證送達、低延遲的場景。
	  
	  Kafka 是 distributed streaming platform，設計給高吞吐量的 event streaming。它用 pull model，訊息會保留一段時間，可以 replay。比較適合 log aggregation、event sourcing、data pipeline。
	  
	  我在不同專案用過兩個。在 CHATISFY 用 RabbitMQ 做 message sending，因為需要複雜的 routing（每個 fanpage 不同 queue）和保證送達。在 Mlytics 用 Kafka 處理高吞吐量的 event streaming，每秒 400K messages 的場景。"
	  
	  **如果被追問：**
	  
	  **Q: "什麼時候用 RabbitMQ，什麼時候用 Kafka？"**
	...
	- Data pipeline
	  ...
	  比如在 CHATISFY，我們需要保證每個 fanpage 的訊息確實送達，不能丟，所以用 RabbitMQ。在 Mlytics Stream，重點是處理大量 events，用 Kafka 更合適。"
	  
	  **Q: "Kafka 的 Consumer Group 是什麼？"**
	  
	  **A:** "Consumer Group 讓多個 consumer 共同消費一個 topic。
	...
	- 用來做 horizontal scaling
	  ...
	  **A:** "主要有四種：
	  ...
	  我在 CHATISFY 可能用 Direct 或 Topic，根據 fanpage ID 做 routing。"
- # DB
- [[Golang 鎖]]
- [[Golang Allocate]]
- [[Algorithm]]
