...
- 【Week 1：基礎 + System Design】
	...
	- 二: MQ + Kafka
	  ├ MQ + Kafka Partition/Rebalance
	- 四: Golang 並發
	  ├─ Slice 陷阱、channel、context
	- 六: 網路 + K8s
	   golang sync、 K8s 基本概念
	- 日: System Design - 匹配 + 排行榜
	  ├─ 下午：匹配系統設計
	  └─ 晚上：排行榜系統、整理 Claude 考題
	- 一: System Design - 聊天
	  ├─ 下午：聊天系統設計
	  └─ 晚上：玩家狀態同步（如果要加）
- Week 2 開始 6.824 + 刷題 ＋ 複習第二輪